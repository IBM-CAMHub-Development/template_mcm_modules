...
resource "null_resource" "import_icp" {
  ...
  provisioner "remote-exec" {
    inline = [    
      ...
      "echo /var/lib/registry/mcm_scripts/mcm_import.sh -u ${var.ocp_admin_user} -ru ${var.rhsm_user} -osu ${var.ocp_server_url} -rh ${var.cluster_docker_registry_server_name} -rp ${var.cluster_docker_registry_server_port} -ri ${var.cluster_docker_registry_server_ip} -cm ${var.cluster_name} -pa ${var.icp_dir} -hs ${var.mcm_controller_server_name} -hu ${var.mcm_controller_admin_user} -mch ${var.man_cluster_on_hub} -icpu ${var.icp_admin_user} -v ${var.icp_inception_image} -s ${var.icp_server_url}",
      ...
      "bash -c '/var/lib/registry/mcm_scripts/mcm_import.sh -u ${var.ocp_admin_user} -ru ${var.rhsm_user} -rp ${var.rhsm_password} -osu ${var.ocp_server_url} -p ${var.ocp_admin_pass} -ri ${var.cluster_docker_registry_server_ip} -rca ${var.cluster_docker_registry_server_ca_crt} -rh ${var.cluster_docker_registry_server_name} -rp ${var.cluster_docker_registry_server_port} -cm ${var.cluster_name} -pa ${var.icp_dir} -hs ${var.mcm_controller_server_name} -hp ${var.mcm_controller_admin_user_password} -hu ${var.mcm_controller_admin_user} -mch ${var.man_cluster_on_hub} -icpu ${var.icp_admin_user} -pw ${var.icp_admin_pass} -s ${var.icp_server_url} -v ${var.icp_inception_image}'"      
